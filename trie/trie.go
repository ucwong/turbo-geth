...
func (t *Trie) PrefixByCumulativeWitnessSize(size uint64, greater bool) (prefix []byte, found bool) {
	if greater {
		return prefixGreaterThanWitnessSize(t.root, size)
	}
	return prefixLowerThanWitnessSize(t.root, size)
}
...
// TODO: this method has same logic with prefixGreaterThanWitnessSize, but it must find lower prefix
func prefixLowerThanWitnessSize(nd node, size uint64) (prefix []byte, found bool) {
	var accumulator uint64 // increase it when go to siblings, don't touch it when go to child
